...
describe('getScriptPubKeyAddress', () => {
	it('reads `address` as returned by Doichain Core 31', () => {
		const tx =
			fixture.responses['blockchain.transaction.get'][
				Object.keys(fixture.responses['blockchain.transaction.get'])[0]
			];
	...
	it('still reads `addresses[0]` from nodes before the fork', () => {
		expect(getScriptPubKeyAddress({ addresses: [fixture.fundedAddress] })).toBe(
