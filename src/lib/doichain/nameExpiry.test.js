...
const REGISTERED_AT = fixture.responses['blockchain.scripthash.get_history'][fixture.nameIndexScripthash][0].height;

const output = (name, height, address = 'N-owner') => ({ height, scriptPubKey: { address, nameOp: { name, name_encoding: 'utf8' } } });

describe('nameExpiry', () => {
	it('keeps a name for 36,000 blocks after its last operation', () => {
		expect(nameExpiry(431320, 431774, DOICHAIN)).toEqual({ confirmed: true, expiresAt: 467320, blocksLeft: 35546, expired: false });
	...
	it('does not guess without a known tip', () => {
		expect(nameExpiry(431320, undefined, DOICHAIN)).toEqual({ confirmed: true, expiresAt: 467320, expired: false });
	});
});

describe('latestNameOperation', () => {
	it('picks the newest operation of that name, unconfirmed ones first', () => {
		const outputs = [output('hello', 100, 'N-old'), output('hello', 300, 'N-new'), output('other', 900), { height: 300, scriptPubKey: {} }];
		expect(latestNameOperation(outputs, 'hello').scriptPubKey.address).toBe('N-new');
		expect(latestNameOperation([...outputs, output('hello', 0, 'N-pending')], 'hello').scriptPubKey.address).toBe('N-pending');
...
describe('name check with expiry', () => {
	...
	it('says until which block a taken name belongs to its owner', async () => {
		...
		expect(result.nameErrorMessage).toContain(`until block ${REGISTERED_AT + 36000}, ${REGISTERED_AT + 36000 - fixture.tipHeight} blocks`);
	});
