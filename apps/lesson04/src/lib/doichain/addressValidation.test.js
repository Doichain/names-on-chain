...
import { DOICHAIN, DOICHAIN_REGTEST } from '@names-on-chain/doichain/doichain.js';
...
describe('address input', () => {
	it('accepts Doichain P2PKH and P2WPKH addresses', () => {
		...
		expect(
			isAddressOf(
				DOICHAIN,
				payments.p2wpkh({ hash: Buffer.alloc(20, 1), network: DOICHAIN }).address
			)
		).toBe(true);
	});
	...
	it('refuses typos, other networks and empty input', () => {
		...
		expect(isAddressOf(DOICHAIN, payments.p2pkh({ hash: Buffer.alloc(20, 1) }).address)).toBe(
			false
		);
	...
	it('takes the address out of a scanned payment URI', () => {
		expect(cleanAddressInput(` doichain:${fixture.fundedAddress}?amount=1 `)).toBe(
