...
import {
	...
	isP2WPKHScript,
	verifiedOutput
} from '@names-on-chain/lesson03/doichain/transactionChecks.js';
import {
	estimateVsize,
	feeFor,
	MAX_INPUTS,
...
export function buildNameRegistrationPsbt(
	...
	const selection = selectCoins(coins, _storageFee, (selected) =>
		feeFor(estimateVsize(selected, outputScripts), feeRate)
	);
	if (selection.error === 'tooFragmented') {
		return {
			error: t('funds.tooFragmented', { address: doichainAddress, max: MAX_INPUTS }),
